...
export class Problem extends Ui{
    ...
    constructor(game, width, height, answer, answer_provider, widgets, widgets_state_handler){
        super(game, width, height, widgets, (ui, time)=>{widgets_state_handler(/**@type {Problem} */(ui), time)})
        this.answer = answer
...
export class TimedProblem extends Problem{
     ...
     */
    static async create(game, src, width, height, answer, answer_provider, time_limit, widgets, widgets_state_handler, on_fail=(problem)=>{}){
     ...
     * @param {boolean} raw_time 
     * @returns {number | {sec: number, min: number, h: number}}
     */
    get_time_left(current_time, raw_time=true){
        if(this.start_time==null) throw new Error('the timed problem hasn\'t started')
     ...
     */
    update(current_time){
        if(this.start_time==null) throw new Error('The timed problem hasn\'t been started properly')
