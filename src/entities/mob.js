...
export class Mob extends Entity {
    ...
    constructor(game, map, tileset, collision_hitbox, combat_hitbox, worldX, worldY, animation_duration, ai, life = null, hitboxes_offset = { combat: { x: 0, y: 0 }, collision: { x: 0, y: 0 } }, bottom_y = null, type = "Mob") {
        super(game, map, tileset, collision_hitbox, combat_hitbox, worldX, worldY, animation_duration, life, hitboxes_offset, bottom_y, false, type)
        ...
        if (this.ai.is_hostile) {
            this.last_attack_time = 0
            if (this.ai.is_rusher) {
                this.last_rush_time = 0
                this.rush_pause_duration = this.ai.get_rush_cooldown() * Math.random()
            ...
            if (this.ai.is_long_range) {
                ...
                this.attack_pause = this.ai.get_attack_cooldown() * 2 * Math.random()
                this.long_range_change_direction_pause = this.ai.get_change_direction_distance_attack_cooldown() * Math.random()
            }
    ...
    manage_states_update(current_time) {
        ...
        if (this.ai.is_hostile) {
            ...
            const d = Math.hypot(
                this.game.get_player().worldX.get() - this.worldX.get(),
                this.game.get_player().worldY.get() - this.worldY.get()
            )
            if (d > this.ai.get_vision_range().get()) {
                // il va direct au bout pr wandering
                // console.log("je m'en vais alors")
            } else if (this.ai.is_long_range && this.ai.is_rusher && this.last_rush_time && this.rush_pause_duration) {
                this.state = constants.WALK_STATE
                ...
                if (d <= this.ai.get_vision_range().get()) {
                    if ((current_time - this.last_rush_time >= this.rush_pause_duration) || (d <= this.ai.get_rush_activation_range().get() && current_time - this.last_rush_time >= this.rush_pause_duration / 2)) {
                        //console.log("je rush")
            ...
            } else if (this.ai.is_rusher && this.last_rush_time && this.rush_pause_duration) {
                this.state = constants.WALK_STATE
    ...
    changeWanderingDirection(current_time) {
        ...
        const speed = this.ai.get_wandering_speed().get()
    ...
    updateWandering(current_time) {
        ...
        if (this.state === constants.WALK_STATE) {
            if (current_time - this.walkStartTime >= this.walkDuration) {
                ...
                if (current_time - this.last_direction_change > this.ai.get_wandering_direction_change_time()) {
                    // console.log("test")
                ...
                if (distance > this.ai.get_wandering_radius().get() * 0.7) {
                    const Dx = dx / Math.max(1, distance)
                    const Dy = dy / Math.max(1, distance)

                    this.dx.set_value(-Dx * this.ai.get_wandering_speed().get())
                    this.dy.set_value(-Dy * this.ai.get_wandering_speed().get())
                }
            }
    ...
    updateLongrangeattacking(current_time) {
        if(!(this.last_attack_time && this.long_range_attack_direction && this.attack_pause && this.long_range_change_direction_pause && this.long_range_last_direction_change)) return
        ...
        const dx_to_player = (this.game.get_player().worldX.get() - this.worldX.get()) / distance * this.ai.get_chasing_speed().get()
        const dy_to_player = (this.game.get_player().worldY.get() - this.worldY.get()) / distance * this.ai.get_chasing_speed().get()
        ...
        let coef = clamp((distance - this.ai.get_distance_attack_range().get()) / this.ai.get_distance_attack_range().get(), -1, 1)
    ...
    updateRushing(current_time) {
        if(!(this.nb_rush_attack_done && this.dash_duration && this.DASH_SPEED && this.last_dash && this.last_attack_time)) return
        const distance = Math.hypot(
            this.game.get_player().worldX.get() - this.worldX.get(),
            this.game.get_player().worldY.get() - this.worldY.get()
        )
        ...
        if (current_time - this.last_dash <= this.dash_duration) {
            const dx = ((this.game.get_player().worldX.get() - this.worldX.get()) / distance) * this.DASH_SPEED.get()
            const dy = ((this.game.get_player().worldY.get() - this.worldY.get()) / distance) * this.DASH_SPEED.get()
            this.dx.set_value(dx)
            this.dy.set_value(dy)
        ...
        if (current_time - this.last_dash > this.dash_duration) {
            ...
            if (current_time - this.last_attack_time > this.ai.get_attack_cooldown() / 6) {
    ...
    updateChasing(current_time) {
        if(!this.last_attack_time) return
        ...
        if (distance < Math.max(this.collision_hitbox.width.get() / 2, this.collision_hitbox.height.get() / 2) + 1) {
            ...
            const dx = (this.game.get_player().worldX.get() - this.worldX.get()) / distance * this.ai.get_chasing_speed().get()
            const dy = (this.game.get_player().worldY.get() - this.worldY.get()) / distance * this.ai.get_chasing_speed().get()
            this.dx.set_value(dx)
            this.dy.set_value(dy)
        }

        if (current_time - this.last_attack_time > this.ai.get_attack_cooldown()) {
            this.attack(current_time)
            this.last_attack_time = current_time
        }
    }
